...
## Overview
kite is a lightweight text editor written mostly in Lua — it aims to provide
something practical, pretty, *small* and fast, implemented as simply as
possible; easy to modify and extend, or to use without doing either.


## Getting Started
When kite is started it's typically opened with a *project directory* — this
is the directory where your project's code and other data resides. The project
directory is set once when kite is started and, for the duration of the
session, cannot be changed.

To open kite with a specific project directory the directory name can be passed
as a command-line argument *(`.` can be passed to use the current directory)* or
the directory can be dragged onto either the kite executable or a running
instance of kite.

The main way of opening files in kite is through the `core:find-file` command
...
## User Module
kite can be configured through use of the user module. The user module can be
...
kite to your personal preference.

The user module is loaded by kite when the application starts, after the plugins
have been loaded.
...
project's directory when kite is started. Project modules can be useful for
things like adding custom commands for project-specific build systems, or
loading project-specific plugins.

The project module is loaded by kite when the application starts, after both the
...
## Commands
Commands in kite are used both through the command finder (`ctrl+shift+p`) and
by kite's keyboard shortcut system. Commands consist of 3 components:
...
## Keymap
All keyboard shortcuts in kite are handled by the `core.keymap` module. A key
binding in kite maps a "stroke" (eg. `ctrl+q`) to one or more commands (eg.
`core:quit`). When the shortcut is pressed kite will iterate each command
...
## Plugins
Plugins in kite are normal lua modules and are treated as such — no
...
plugins will be automatically loaded when kite starts. To uninstall a plugin the
plugin file can be deleted — any plugin (including those included with kite's
...
## Color Themes
Colors themes in kite are lua modules which overwrite the color fields of kite's
